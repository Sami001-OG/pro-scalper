...
interface OptimizeResult {
  best: {
    sl: number; tp: number; leverage: number; margin: number;
    totalTrades: number; winRate: number; totalPnl: number; dollarPnL?: number;
    liquidations?: number;
  };
  top10: Array<{
    sl: number; tp: number; leverage: number; margin: number;
...
export default function App() {
  ...
  const [slMultiplier, setSlMultiplier] = useState(3.1);
  const [tpMultiplier, setTpMultiplier] = useState(0.5);
  ...
  const runBacktest = async () => {
    ...
    try {
      const res = await fetch(`/api/backtest?leverage=${leverage}&sl=${slMultiplier}&tp=${tpMultiplier}&capital=${totalCapital}&margin=${marginPct}`);
      if (!res.ok) {
  ...
  const runOptimize = async () => {
    ...
    setBacktestData(null);
    try {
      const res = await fetch(`/api/optimize?capital=${totalCapital}`);
      if (!res.ok) {
        throw new Error(`API returned ${res.status}`);
      }
      const data = await res.json();
      setOptimizeData({
        best: data[0] || { sl: 0, tp: 0, totalTrades: 0, winRate: 0, totalPnl: 0, leverage: 0, margin: 0 },
        top10: data
      });
    } catch (e) {
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-neutral-800 pb-6 gap-4">
          <div>
            ...
            <p className="text-neutral-400 mt-1">Bitcoin Perpetual Trading Bot (BTCUSDT)</p>
          </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex flex-wrap gap-3">
                <div className="flex items-center gap-2 bg-neutral-900 border border-neutral-800 rounded-lg px-3 py-2">
                  <label className="text-xs text-neutral-400">Capital:</label>
                  <input 
                    type="number" 
              ...
              <div className="space-y-6">
                <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-xl">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-neutral-950 p-4 rounded-lg border border-neutral-800">
                      <p className="text-sm text-neutral-400">Leverage</p>
                      <p className="text-xl font-bold">{optimizeData.best.leverage}x</p>
                    </div>
                    <div className="bg-neutral-950 p-4 rounded-lg border border-neutral-800">
                      <p className="text-sm text-neutral-400">Margin</p>
                      <p className="text-xl font-bold">{optimizeData.best.margin}%</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <StatusCard title="Net Profit" value={`$${optimizeData.best.dollarPnL?.toFixed(2)} (+${optimizeData.best.totalPnl.toFixed(2)}%)`} valueColor={optimizeData.best.totalPnl > 0 ? 'text-green-500' : 'text-red-500'} icon={<DollarSign className="text-neutral-400" />} />
                  </div>
                </div>
                    ...
                    <table className="w-full text-left text-sm">
                      <thead className="bg-neutral-950/50 text-neutral-400 border-b border-neutral-800">
                        <tr>
                          ...
                          <th className="px-6 py-4 font-medium">Lev / Margin</th>
                          <th className="px-6 py-4 font-medium">Trades / Liq</th>
                        ...
                        {optimizeData.top10.map((res, idx) => (
                          <tr key={idx} className="hover:bg-neutral-800/20 transition-colors">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-neutral-400">
                              {res.leverage}x / {res.margin}%
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div>{res.totalTrades}</div>
                              {res.liquidations !== undefined && res.liquidations > 0 && (
                                <div className="text-[10px] text-red-500">Liq: {res.liquidations}</div>
                              )}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">{res.winRate.toFixed(1)}%</td>
                            <td className="px-6 py-4 whitespace-nowrap font-mono text-green-500">
                              ${res.dollarPnL?.toFixed(2)} (+{res.totalPnl.toFixed(2)}%)
                            </td>
                          </tr>
                        ))}
                      </tbody>
              ...
              <>
                <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-xl mb-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <StatusCard title="Liquidations" value={backtestData.liquidations} valueColor={backtestData.liquidations > 0 ? 'text-red-500' : 'text-green-500'} icon={<AlertCircle className="text-neutral-400" />} />
                    <StatusCard 
                      title="Net Profit" 
                      value={`$${backtestData.totalDollarPnL.toFixed(2)} (${((backtestData.totalDollarPnL / (backtestData.initialCapital || 1000)) * 100).toFixed(2)}%)`} 
                      valueColor={backtestData.totalDollarPnL > 0 ? 'text-green-500' : 'text-red-500'} 
                      icon={<DollarSign className="text-neutral-400" />} 
                    />
                  </div>
                </div>
      ...
      <table className="w-full text-left text-sm">
        <thead className="bg-neutral-950/50 text-neutral-400 border-b border-neutral-800">
          <tr>
            ...
            <th className="px-6 py-4 font-medium">Entry / Liq</th>
            <th className="px-6 py-4 font-medium">SL / TP (%)</th>
        ...
        <tbody className="divide-y divide-neutral-800/50">
          {signals.map((signal) => (
            <tr key={signal.id} className="hover:bg-neutral-800/20 transition-colors">
              ...
              <td className="px-6 py-4 whitespace-nowrap font-mono">
                <div className="text-neutral-200">${signal.entryPrice.toFixed(2)}</div>
                {signal.liquidationPrice && (
                  <div className="text-[10px] text-red-600 font-bold uppercase tracking-tighter">Liq: ${signal.liquidationPrice.toFixed(2)}</div>
                )}
              </td>
              <td className="px-6 py-4 whitespace-nowrap font-mono text-xs">
                <div className="text-red-400">SL: ${signal.stopLoss.toFixed(2)} {signal.slMult ? `(${signal.slMult.toFixed(1)}%)` : ''}</div>
                <div className="text-green-400">TP: ${signal.tp.toFixed(2)} {signal.tpMult ? `(${signal.tpMult.toFixed(1)}%)` : ''}</div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
              ...
              <td className={`px-6 py-4 whitespace-nowrap font-mono font-medium ${
                ...
                {signal.dollarPnL !== undefined && (
                  <span className="text-xs opacity-70 block">
                    {signal.dollarPnL > 0 ? '+$' : '-$'}{Math.abs(signal.dollarPnL).toFixed(2)}
                  </span>
                )}
              </td>
            </tr>
