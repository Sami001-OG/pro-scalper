...
const TAKER_FEE_RATE = 0.0004; // 0.04% (Standard Binance Taker fee)
const SLIPPAGE_RATE = 0.0001; // 0.01% realistic slippage
const MAINTENANCE_MARGIN = 0.004; // 0.4% maintenance margin
const LEVERAGE = 20; // Default leverage for signals

// Helper to calculate net PnL percentage relative to margin
export function calculateNetPnLPct(priceChangePct: number, leverage: number) {
  const frictionPerSide = TAKER_FEE_RATE + SLIPPAGE_RATE;
  const priceChangeDecimal = priceChangePct / 100;
  const entryFrictionPct = leverage * frictionPerSide;
  const exitFrictionPct = leverage * (1 + priceChangeDecimal) * frictionPerSide;
  return (priceChangePct * leverage) - ((entryFrictionPct + exitFrictionPct) * 100);
}

// Calculate the percentage distance to liquidation where Equity = Maintenance Margin
export function calculateLiqDistPct(leverage: number) {
  const frictionPerSide = TAKER_FEE_RATE + SLIPPAGE_RATE;
  const estimatedFees = leverage * frictionPerSide * 2; 
  return (100 * (1 - MAINTENANCE_MARGIN - estimatedFees)) / leverage;
}
...
export let currentSlMult = 3.1;
export let currentTpMult = 0.5;
  ...
  try {
    for (const symbol of SYMBOLS) {
      ...
      if (trend === 'BULLISH' && currentPrice > currentVWAP && stochCrossUpOverSold && currentEWO > 0) {
        ...
        const effectiveSlPct = Math.min(currentSlMult, liqDist * 0.99);
        ...
        signal = {
          ...
          stopLoss: currentPrice * (1 - (effectiveSlPct/100)),
          tp: currentPrice * (1 + (currentTpMult/100)),
          status: 'OPEN',
          pnl: 0,
...
function manageActiveTrades(symbol: string, currentPrice: number) {
  for (let i = activeTrades.length - 1; i >= 0; i--) {
    ...
    if (trade.type === 'LONG') {
      const priceChangePct = ((currentPrice - trade.entryPrice) / trade.entryPrice) * 100;
      trade.pnl = calculateNetPnLPct(priceChangePct, LEVERAGE); // LEVERAGE is 20 for signals
      ...
      const priceChangePct = ((trade.entryPrice - currentPrice) / trade.entryPrice) * 100;
      trade.pnl = calculateNetPnLPct(priceChangePct, LEVERAGE);
  ...
  startBinanceWS({
    onKlineUpdate: (kline: LiveKline) => {
      if (kline.timeframe === '5m') { // Match new timeframe
        ...
        // Trigger market analysis on every tick for exact notification
