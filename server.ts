...
async function startServer() {
  ...
  app.get('/api/backtest', async (req, res) => {
    try {
      const slMultiplier = req.query.sl ? parseFloat(req.query.sl as string) : 3.1;
      const tpMultiplier = req.query.tp ? parseFloat(req.query.tp as string) : 0.5;
      const leverage = req.query.leverage ? parseFloat(req.query.leverage as string) : 1;
      const capital = req.query.capital ? parseFloat(req.query.capital as string) : 1000;
      const margin = req.query.margin ? parseFloat(req.query.margin as string) : 10;

      const results = await runBacktest(slMultiplier, tpMultiplier, leverage, capital, margin);
      res.json(results);
    } catch (error) {
  ...
  app.get('/api/optimize', async (req, res) => {
    try {
      const capital = req.query.capital ? parseFloat(req.query.capital as string) : 1000;
      const results = await runBruteForceOptimization(capital);
      res.json(results);
    } catch (error) {
  ...
  const server = app.listen(PORT as any, '0.0.0.0', () => {
